...
percent_dict={'7d':'percent_change_7d','24h':'percent_change_24h','1h':'percent_change_1h'}
def plot_percent_change(percent_timeframe):
    if sort_values=='yes':
        df_change=df_change.sort_values(by=[percent_dict[percent_timeframe]])
    col3.write(percent_dict[percent_timeframe])
    plt.figure(figsize=(5, 25))
    plt.subplots_adjust(top=1, bottom=0)
    df_change[percent_dict[percent_timeframe]].plot(kind='barh',color=df_change[percent_dict[percent_timeframe]].map({True: g, False: r}))
    col3.pyplot(plt)

plot_percent_change(percent_timeframe)
